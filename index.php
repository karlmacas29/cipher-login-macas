    ...
    if(!$user && !$password){
      echo 'error';
        ...
        if("block" == $st){//check if this account is blocked.
          ...
          $_SESSION['id'] = $id;
          $_SESSION['name'] = $name;
          $_SESSION['code'] = $_POST['code']; //code for google auth
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container position-relative py-4 my-4" style="height: 90vh;" id="darkbg">
    ...
    <div class="modal fade" id="regPop1">
          ...
          <!--toast -->
            ...
            <div id="myToast" class="toast border-0 position-fixed top-0 top-0 end-0 m-2" role="alert" aria-live="assertive" aria-atomic="true">
            <div class="d-flex">
              <div class="toast-body mt-3">
                  <p id="err-txt" class="text-center fs-6"></p>
              </div>
       ...
       $(document).ready(function(){
        ...
        $("#register1").click(function(){
          ...
          success: function (res) {
            if (res['val'] == true) {
              var text = res['url'];
              var textname = res['uname'];
              var secretCd = res['secret'];
              ...
              Swal.fire({
                ...
                '<input type="text" class="form-control" id="text-code">' +
                '<div class="d-flex justify-content-center gap-1 mt-2">'+
                '<button class="btn btn-primary" id="subNM"><span class="" id="loadC"></span> Submit</button>' +
                '</div>',
                ...
                $("#subNM").on("click", function(){
                  var nmData = $("#text-code").val();
                  var data_com = {
                    sc_qr : secretCd ,
                    cd_qr : nmData ,
                    nm_qr : textname
                  ...
                  $("#loadC").addClass("spinner-border text-light spinner-border-sm");
                  $("#subNM").addClass("disabled");

                  setTimeout(function () {
                    $("#loadC").removeClass("spinner-border text-light spinner-border-sm");
                    $("#subNM").removeClass("disabled");
                  }, 2000);
                  ...
                  $.ajax({
                    ...
                    data: data_com, 
                    dataType: "JSON",
                    url: "./confirm-auth.php",
                    success: function(r) {
                      if (r['val'] == false){     
                        var err_msg = r['msg'];
                        $("#myToast").toast("show").addClass("text-bg-danger").removeClass("text-bg-success");
                        $("#err-txt").html("<i class='bi bi-exclamation-circle-fill'></i> "+err_msg);
                      }else{
                        var err_msg = r['msg'];
                        $("#myToast").toast("show").addClass("text-bg-success").removeClass("text-bg-danger");
                        $("#err-txt").html("<i class='bi bi-check-circle'></i> "+err_msg);
                        setTimeout(function() {
                          location.reload();
                        },2500);
                      }
